...
class model(nn.Module):
    def __init__(self,parameters):                      #parameters为hyperparameters的实例
        ...
        self.bilstm = nn.LSTM(self.embed_dim, self.hidden_dim // 2, dropout=self.parameter.dropout,
    ...
    def forward(self, x):
        ...
        # print(self.hidden[0].size())0
        lstm_out, self.hidden = self.bilstm(embed, self.hidden)
        ...
        lstm_out = F.tanh(lstm_out)
        # linear
        logit = self.hidden2label1(lstm_out)
